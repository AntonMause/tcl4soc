...
#
# Emcraft Starter Kit for Microsemi M2S050-FGG896 
...
# 
set PATH_SOURCES   .
set PATH_LINKED    ../../11p7/g4start5s_lnk
set PATH_IMPORTED  ../../11p7/g4start5s_src
...
# create new project
new_project -location $PATH_LINKED -name {g4start5s_lnk} -project_description {G4 M2S050 Emcraft Starter Kit} \
	-block_mode 0 -standalone_peripheral_initialization 0 -use_enhanced_constraint_flow 1 -hdl {VHDL} \
	-family {SmartFusion2} -die {M2S050T} -package {896 FBGA} -speed {STD} -die_voltage {1.2} \
...
save_project_as -location $PATH_IMPORTED -name {g4start5s_src} -replace_links 1 -files {all} -designer_views {all} 
save_project 

# copy project to ZIP archive
project_archive -location $PATH_LINKED -name {g4start5s_src} -replace_links 1 -files {all} -designer_views {all} 
save_project 
