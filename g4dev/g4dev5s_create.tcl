...
#
# Development Kit for Microsemi M2S050T-1FGG896 (2012)
# Board populated with and used as SmartFusion2 with Transceiver
# huge PCB, EOL 2015, succesor Advenced DevKit
#
# tested with board Rev D silicon Rev 0
#

# 
set PATH_SOURCES   .
set PATH_LINKED    ../../11p7/g4dev5s_lnk
set PATH_IMPORTED  ../../11p7/g4dev5s_src
...
# create new project
new_project -location $PATH_LINKED -name {g4dev5s_lnk} -project_description {G4 M2S050T Development} \
	-block_mode 0 -standalone_peripheral_initialization 0 -use_enhanced_constraint_flow 1 -hdl {VHDL} \
	-family {SmartFusion2} -die {M2S050T} -package {896 FBGA} -speed {-1} -die_voltage {1.2} \
...
save_project_as -location $PATH_IMPORTED -name {g4dev5s_src} -replace_links 1 -files {all} -designer_views {all} 
save_project 

# copy project to ZIP archive
project_archive -location $PATH_LINKED -name {g4dev5s_src} -replace_links 1 -files {all} -designer_views {all} 
save_project 
